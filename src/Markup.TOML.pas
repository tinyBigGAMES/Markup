...
type
  ...
  TMuTomlValue = record
  private
    FKind:          TMuTomlValueKind;
    FStringValue:   string;
    FIntegerValue:  Int64;
    FFloatValue:    Double;
    FBooleanValue:  Boolean;
    FDateTimeValue: TDateTime;
    FArrayValue:    TMuTomlArray;
    FTableValue:    TMuToml;
  public
    class function CreateNone(): TMuTomlValue; static;
    class function CreateString(const AValue: string): TMuTomlValue; static;
    class function CreateInteger(const AValue: Int64): TMuTomlValue; static;
    class function CreateFloat(const AValue: Double): TMuTomlValue; static;
    class function CreateBoolean(const AValue: Boolean): TMuTomlValue; static;
    class function CreateDateTime(const AValue: TDateTime): TMuTomlValue; static;
    class function CreateDate(const AValue: TDateTime): TMuTomlValue; static;
    class function CreateTime(const AValue: TDateTime): TMuTomlValue; static;
    class function CreateArray(const AValue: TMuTomlArray): TMuTomlValue; static;
    class function CreateTable(const AValue: TMuToml): TMuTomlValue; static;

    property Kind: TMuTomlValueKind read FKind;
    ...
    function AsArray(): TMuTomlArray;
    function AsTable(): TMuToml;
  ...
  TMuTomlArray = class
  private
    FItems: TList<TMuTomlValue>;
    function GetCount(): Integer;
    function GetItem(const AIndex: Integer): TMuTomlValue;
  public
    constructor Create();
    destructor Destroy(); override;

    procedure Add(const AValue: TMuTomlValue);
    ...
    property Items[const AIndex: Integer]: TMuTomlValue read GetItem; default;
  end;

  { TMuTomlParseError }
  TMuTomlParseError = class(Exception)
  private
    FLine:   Integer;
    FColumn: Integer;
  public
  ...
  TMuToml = class
  private
    FValues:         TDictionary<string, TMuTomlValue>;
    FOwnedTables:    TObjectList<TMuToml>;
    FOwnedArrays:    TObjectList<TMuTomlArray>;
    ...
    FLength:         Integer;
    FLastError:      string;
    FCurrentTable:   TMuToml;
    FImplicitTables: TDictionary<string, TMuToml>;
    FDefinedTables:  TDictionary<string, Boolean>;
    FArrayTables:    TDictionary<string, TMuTomlArray>;
    ...
    procedure ParseKeyValue(const ATable: TMuToml);
    ...
    function  ParseValue(): TMuTomlValue;
    function  ParseNumberOrDateTime(): TMuTomlValue;
    function  ParseInteger(const AText: string): Int64;
    function  ParseFloat(const AText: string): Double;
    function  ParseDateTime(const AText: string): TMuTomlValue;
    function  ParseArray(): TMuTomlArray;
    function  ParseInlineTable(): TMuToml;
    ...
    function  GetOrCreateTablePath(const APath: TArray<string>;
      const AImplicit: Boolean): TMuToml;
    function  GetOrCreateArrayTablePath(
      const APath: TArray<string>): TMuToml;
    procedure SetValueAtPath(const ATable: TMuToml;
      const APath: TArray<string>; const AValue: TMuTomlValue);
    function  PathToString(const APath: TArray<string>): string;

  public
    // Ownership
    function CreateOwnedTable(): TMuToml;
    function CreateOwnedArray(): TMuTomlArray;

    constructor Create();
    destructor Destroy(); override;

    // Parsing
    class function FromFile(const AFilename: string): TMuToml;
    class function FromString(const ASource: string): TMuToml;
    function Parse(const ASource: string): Boolean;
    function GetLastError(): string;

    // Value access
    function  TryGetValue(const AKey: string;
      out AValue: TMuTomlValue): Boolean;
    ...
    function GetOrCreateTable(const AKey: string): TMuToml;
    function GetOrCreateArray(const AKey: string): TMuTomlArray;
    function GetTable(const AKey: string): TMuToml;
    function GetArray(const AKey: string): TMuTomlArray;
    ...
    procedure SetValue(const AKey: string; const AValue: TMuTomlValue);
...
class function TMuTomlValue.CreateNone(): TMuTomlValue;
begin
  Result       := Default(TMuTomlValue);
  Result.FKind := tvkNone;
end;

class function TMuTomlValue.CreateString(
  const AValue: string): TMuTomlValue;
begin
  Result              := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateInteger(
  const AValue: Int64): TMuTomlValue;
begin
  Result               := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateFloat(
  const AValue: Double): TMuTomlValue;
begin
  Result             := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateBoolean(
  const AValue: Boolean): TMuTomlValue;
begin
  Result               := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateDateTime(
  const AValue: TDateTime): TMuTomlValue;
begin
  Result                := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateDate(
  const AValue: TDateTime): TMuTomlValue;
begin
  Result                := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateTime(
  const AValue: TDateTime): TMuTomlValue;
begin
  Result                := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateArray(
  const AValue: TMuTomlArray): TMuTomlValue;
begin
  Result             := Default(TMuTomlValue);
...
class function TMuTomlValue.CreateTable(
  const AValue: TMuToml): TMuTomlValue;
begin
  Result             := Default(TMuTomlValue);
...
function TMuTomlValue.AsString(): string;
begin
...
function TMuTomlValue.AsInteger(): Int64;
begin
...
function TMuTomlValue.AsFloat(): Double;
begin
...
function TMuTomlValue.AsBoolean(): Boolean;
begin
...
function TMuTomlValue.AsDateTime(): TDateTime;
begin
...
function TMuTomlValue.AsArray(): TMuTomlArray;
begin
...
function TMuTomlValue.AsTable(): TMuToml;
begin
...
function TMuTomlValue.IsNone(): Boolean;
begin
  Result := FKind = tvkNone;
end;
...
constructor TMuTomlArray.Create();
begin
  inherited;
  FItems := TList<TMuTomlValue>.Create();
end;

destructor TMuTomlArray.Destroy();
begin
  FItems.Free();
  inherited;
end;

function TMuTomlArray.GetCount(): Integer;
begin
  Result := FItems.Count;
end;

function TMuTomlArray.GetItem(const AIndex: Integer): TMuTomlValue;
begin
  Result := FItems[AIndex];
end;

procedure TMuTomlArray.Add(const AValue: TMuTomlValue);
begin
  FItems.Add(AValue);
end;

procedure TMuTomlArray.AddString(const AValue: string);
begin
  FItems.Add(TMuTomlValue.CreateString(AValue));
end;

procedure TMuTomlArray.AddInteger(const AValue: Int64);
begin
  FItems.Add(TMuTomlValue.CreateInteger(AValue));
end;

procedure TMuTomlArray.AddFloat(const AValue: Double);
begin
  FItems.Add(TMuTomlValue.CreateFloat(AValue));
end;

procedure TMuTomlArray.AddBoolean(const AValue: Boolean);
begin
  FItems.Add(TMuTomlValue.CreateBoolean(AValue));
end;

procedure TMuTomlArray.Clear();
begin
  FItems.Clear();
end;
...
constructor TMuTomlParseError.Create(const AMessage: string;
...
constructor TMuToml.Create();
begin
  inherited;
  FValues         := TDictionary<string, TMuTomlValue>.Create();
  FOwnedTables    := TObjectList<TMuToml>.Create(True);
  FOwnedArrays    := TObjectList<TMuTomlArray>.Create(True);
...
class function TMuToml.FromFile(const AFilename: string): TMuToml;
var
  LSource: string;
begin
  if not TFile.Exists(AFilename) then
    raise TMuTomlParseError.Create('File not found: ' + AFilename, 0, 0);

  LSource := TFile.ReadAllText(AFilename, TEncoding.UTF8);
  Result  := FromString(LSource);
end;

class function TMuToml.FromString(const ASource: string): TMuToml;
begin
  Result := TMuToml.Create();
  try
    if not Result.Parse(ASource) then
      raise TMuTomlParseError.Create(Result.GetLastError(),
...
function TMuToml.Parse(const ASource: string): Boolean;
begin
  FSource       := ASource;
  FPos          := 1;
  FLine         := 1;
  ...
  FImplicitTables := TDictionary<string, TMuToml>.Create();
  FDefinedTables  := TDictionary<string, Boolean>.Create();
  FArrayTables    := TDictionary<string, TMuTomlArray>.Create();

  try
    Result := ParseDocument();
  except
    on E: TMuTomlParseError do
    begin
...
function TMuToml.GetLastError(): string;
begin
  Result := FLastError;
end;
...
function TMuToml.IsEOF(): Boolean;
begin
  Result := FPos > FLength;
end;

function TMuToml.Peek(const AOffset: Integer): Char;
var
  LIndex: Integer;
begin
  LIndex := FPos + AOffset;
...
procedure TMuToml.Advance(const ACount: Integer);
var
  LI: Integer;
begin
...
procedure TMuToml.SkipWhitespace();
begin
  while not IsEOF() and IsWhitespace(Peek()) do
    Advance();
end;

procedure TMuToml.SkipWhitespaceAndNewlines();
begin
  while not IsEOF() do
  begin
...
procedure TMuToml.SkipToEndOfLine();
begin
  while not IsEOF() and not IsNewline(Peek()) do
    Advance();
end;

function TMuToml.IsNewline(const ACh: Char): Boolean;
begin
  Result := (ACh = #10) or (ACh = #13);
end;

function TMuToml.IsWhitespace(const ACh: Char): Boolean;
begin
  Result := (ACh = ' ') or (ACh = #9);
end;

function TMuToml.IsBareKeyChar(const ACh: Char): Boolean;
begin
  Result := CharInSet(ACh, ['A'..'Z', 'a'..'z', '0'..'9', '_', '-']);
end;

function TMuToml.IsDigit(const ACh: Char): Boolean;
begin
  Result := CharInSet(ACh, ['0'..'9']);
end;

function TMuToml.IsHexDigit(const ACh: Char): Boolean;
begin
  Result := CharInSet(ACh, ['0'..'9', 'A'..'F', 'a'..'f']);
end;

function TMuToml.IsOctalDigit(const ACh: Char): Boolean;
begin
  Result := CharInSet(ACh, ['0'..'7']);
end;

function TMuToml.IsBinaryDigit(const ACh: Char): Boolean;
begin
  Result := CharInSet(ACh, ['0', '1']);
end;

function TMuToml.HexValue(const ACh: Char): Integer;
begin
...
procedure TMuToml.Error(const AMessage: string);
begin
  raise TMuTomlParseError.Create(AMessage, FLine, FColumn);
end;
...
function TMuToml.CreateOwnedTable(): TMuToml;
begin
  Result := TMuToml.Create();
  FOwnedTables.Add(Result);
end;

function TMuToml.CreateOwnedArray(): TMuTomlArray;
begin
  Result := TMuTomlArray.Create();
  FOwnedArrays.Add(Result);
end;

// -- Parser methods -----------------------------------------------------------

procedure TMuToml.CaptureComment();
var
  LStart: Integer;
  LLine:  string;
begin
...
function TMuToml.ParseDocument(): Boolean;
begin
  Result := True;
...
procedure TMuToml.ParseExpression();
begin
  SkipWhitespace();
...
procedure TMuToml.ParseKeyValue(const ATable: TMuToml);
var
  LKey:   TArray<string>;
  LValue: TMuTomlValue;
begin
...
function TMuToml.ParseKey(): TArray<string>;
var
  LParts: TList<string>;
  LKey:   string;
begin
...
function TMuToml.ParseSimpleKey(): string;
begin
  if Peek() = '"' then
...
function TMuToml.ParseBasicString(): string;
begin
  if Peek() <> '"' then
    Error('Expected ''"''');
  Advance();
...
function TMuToml.ParseMultiLineBasicString(): string;
var
...
function TMuToml.ParseLiteralString(): string;
begin
  if Peek() <> '''' then
    Error('Expected ''''');
  Advance();
...
function TMuToml.ParseMultiLineLiteralString(): string;
begin
  Result := '';
...
function TMuToml.ParseEscapeSequence(): string;
var
  LCodePoint: Integer;
  LI:         Integer;
begin
...
function TMuToml.ParseValue(): TMuTomlValue;
var
  LCh: Char;
begin
  LCh := Peek();
  ...
  if LCh = '"' then
    Result := TMuTomlValue.CreateString(ParseBasicString())
  else if LCh = '''' then
    Result := TMuTomlValue.CreateString(ParseLiteralString())
  else if LCh = '[' then
    Result := TMuTomlValue.CreateArray(ParseArray())
  else if LCh = '{' then
    Result := TMuTomlValue.CreateTable(ParseInlineTable())
  else if (LCh = 't') and (Peek(1) = 'r') and (Peek(2) = 'u') and
          (Peek(3) = 'e') and not IsBareKeyChar(Peek(4)) then
  begin
    Advance(4);
    Result := TMuTomlValue.CreateBoolean(True);
  end
  ...
  begin
    Advance(5);
    Result := TMuTomlValue.CreateBoolean(False);
  end
  ...
  begin
    Error('Invalid value');
    Result := TMuTomlValue.CreateNone();
  end;
end;

function TMuToml.ParseNumberOrDateTime(): TMuTomlValue;
var
  LStart:    Integer;
  LText:     string;
  LHasColon: Boolean;
...
begin
  ...
  begin
    ...
    begin
      Advance(3);
      Result := TMuTomlValue.CreateFloat(Infinity);
      Exit;
    end
    else if (Peek() = 'n') and (Peek(1) = 'a') and (Peek(2) = 'n') then
    begin
      Advance(3);
      Result := TMuTomlValue.CreateFloat(NaN);
      Exit;
    end
    else
      Error('Invalid value');
  ...
  begin
    ...
    begin
      ...
      begin
        Advance(4);
        Result := TMuTomlValue.CreateFloat(NegInfinity);
        Exit;
      end
  ...
  if LHasColon or (LHasDash and (Length(LText) >= 10)) then
    ...
    Result := TMuTomlValue.CreateFloat(ParseFloat(LText))
  else
    Result := TMuTomlValue.CreateInteger(ParseInteger(LText));
end;

function TMuToml.ParseInteger(const AText: string): Int64;
var
  LText: string;
  LNeg:  Boolean;
  LI:    Integer;
...
function TMuToml.ParseFloat(const AText: string): Double;
var
...
function TMuToml.ParseDateTime(const AText: string): TMuTomlValue;
var
  LText:        string;
  LYear:        Integer;
  LMonth:       Integer;
...
begin
  ...
  if LHasDate and LHasTime then
    Result := TMuTomlValue.CreateDateTime(LDateTime)
  else if LHasDate then
    Result := TMuTomlValue.CreateDate(LDateTime)
  else
    Result := TMuTomlValue.CreateTime(LDateTime);
end;

function TMuToml.ParseArray(): TMuTomlArray;
var
  LValue: TMuTomlValue;
begin
  if Peek() <> '[' then
...
function TMuToml.ParseInlineTable(): TMuToml;
var
  LKey:   TArray<string>;
  LValue: TMuTomlValue;
begin
...
procedure TMuToml.ParseTableHeader();
var
  LKey:  TArray<string>;
  LPath: string;
begin
...
procedure TMuToml.ParseArrayTableHeader();
var
  LKey:  TArray<string>;
  LPath: string;
begin
...
function TMuToml.PathToString(const APath: TArray<string>): string;
var
  LI: Integer;
begin
  Result := '';
...
function TMuToml.GetOrCreateTablePath(const APath: TArray<string>;
  const AImplicit: Boolean): TMuToml;
var
  LCurrent:  TMuToml;
  LI:        Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      LCurrent.FValues.Add(LKey, TMuTomlValue.CreateTable(LNewTable));
...
function TMuToml.GetOrCreateArrayTablePath(
  const APath: TArray<string>): TMuToml;
var
  LCurrent:  TMuToml;
  LI:        Integer;
...
begin
  ...
  begin
    ...
    LCurrent.FValues.Add(LKey, TMuTomlValue.CreateArray(LArray));
    if not LCurrent.FKeyOrder.Contains(LKey) then
      LCurrent.FKeyOrder.Add(LKey);
  ...
  LNewTable := CreateOwnedTable();
  LArray.Add(TMuTomlValue.CreateTable(LNewTable));

  Result := LNewTable;
end;

procedure TMuToml.SetValueAtPath(const ATable: TMuToml;
  const APath: TArray<string>; const AValue: TMuTomlValue);
var
  LCurrent:  TMuToml;
  LI:        Integer;
...
function TMuToml.GetKeyComment(const AKey: string): string;
begin
  if not FKeyComments.TryGetValue(AKey, Result) then
    Result := '';
end;

procedure TMuToml.SetKeyComment(const AKey: string; const AValue: string);
begin
...
function TMuToml.TryGetValue(const AKey: string;
  out AValue: TMuTomlValue): Boolean;
begin
  Result := FValues.TryGetValue(AKey, AValue);
end;

function TMuToml.ContainsKey(const AKey: string): Boolean;
begin
  Result := FValues.ContainsKey(AKey);
end;

procedure TMuToml.RemoveKey(const AKey: string);
begin
  FValues.Remove(AKey);
  FKeyOrder.Remove(AKey);
end;

function TMuToml.GetString(const AKey: string;
  const ADefault: string): string;
var
  LValue: TMuTomlValue;
begin
...
function TMuToml.GetInteger(const AKey: string;
...
function TMuToml.GetFloat(const AKey: string;
...
function TMuToml.GetBoolean(const AKey: string;
...
function TMuToml.GetDateTime(const AKey: string;
...
procedure TMuToml.SetString(const AKey: string; const AValue: string);
begin
  FValues.AddOrSetValue(AKey, TMuTomlValue.CreateString(AValue));
  if not FKeyOrder.Contains(AKey) then
    FKeyOrder.Add(AKey);
end;

procedure TMuToml.SetInteger(const AKey: string; const AValue: Int64);
begin
  FValues.AddOrSetValue(AKey, TMuTomlValue.CreateInteger(AValue));
  if not FKeyOrder.Contains(AKey) then
    FKeyOrder.Add(AKey);
end;

procedure TMuToml.SetFloat(const AKey: string; const AValue: Double);
begin
  FValues.AddOrSetValue(AKey, TMuTomlValue.CreateFloat(AValue));
  if not FKeyOrder.Contains(AKey) then
    FKeyOrder.Add(AKey);
end;

procedure TMuToml.SetBoolean(const AKey: string; const AValue: Boolean);
begin
  FValues.AddOrSetValue(AKey, TMuTomlValue.CreateBoolean(AValue));
  if not FKeyOrder.Contains(AKey) then
    FKeyOrder.Add(AKey);
end;

procedure TMuToml.SetDateTime(const AKey: string; const AValue: TDateTime);
begin
  FValues.AddOrSetValue(AKey, TMuTomlValue.CreateDateTime(AValue));
  if not FKeyOrder.Contains(AKey) then
    FKeyOrder.Add(AKey);
end;

function TMuToml.GetOrCreateTable(const AKey: string): TMuToml;
var
  LValue:    TMuTomlValue;
  LNewTable: TMuToml;
begin
  ...
  begin
    LNewTable := CreateOwnedTable();
    FValues.Add(AKey, TMuTomlValue.CreateTable(LNewTable));
    if not FKeyOrder.Contains(AKey) then
      FKeyOrder.Add(AKey);
    Result := LNewTable;
  end;
...
function TMuToml.GetOrCreateArray(const AKey: string): TMuTomlArray;
var
  LValue:    TMuTomlValue;
  LNewArray: TMuTomlArray;
begin
  if FValues.TryGetValue(AKey, LValue) then
  begin
    if LValue.Kind = tvkArray then
  ...
  begin
    ...
    FValues.Add(AKey, TMuTomlValue.CreateArray(LNewArray));
    if not FKeyOrder.Contains(AKey) then
      FKeyOrder.Add(AKey);
    Result := LNewArray;
  end;
...
function TMuToml.GetTable(const AKey: string): TMuToml;
var
  LValue: TMuTomlValue;
begin
...
function TMuToml.GetArray(const AKey: string): TMuTomlArray;
var
  LValue: TMuTomlValue;
begin
...
function TMuToml.GetKeys(): TArray<string>;
begin
  Result := FKeyOrder.ToArray();
end;

function TMuToml.GetCount(): Integer;
begin
  Result := FValues.Count;
end;

procedure TMuToml.SetValue(const AKey: string;
  const AValue: TMuTomlValue);
begin
