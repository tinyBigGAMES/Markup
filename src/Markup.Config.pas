...
destructor TMuConfig.Destroy();
begin
  Clear();
  inherited;
end;

procedure TMuConfig.Clear();
begin
...
function TMuConfig.GetLastError(): string;
begin
  Result := FLastError;
end;

function TMuConfig.NavigateToTable(const AKeyPath: string;
  const ACreateMissing: Boolean; out ATable: TMuToml;
  out AFinalKey: string): Boolean;
var
...
function TMuConfig.GetValueAtPath(const AKeyPath: string;
  out AValue: TMuTomlValue): Boolean;
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.LoadFromFile(const AFilename: string): Boolean;
begin
  Result := False;
  Clear();
  ...
  try
    FToml     := TMuToml.FromFile(AFilename);
    FFilename := AFilename;
    Result    := True;
  except
    on E: Exception do
...
function TMuConfig.LoadFromString(const ASource: string): Boolean;
begin
  Result := False;
  Clear();

  try
    FToml  := TMuToml.FromString(ASource);
    Result := True;
  except
    on E: Exception do
    begin
...
function TMuConfig.EscapeString(const AValue: string): string;
begin
  Result := AValue;
...
procedure TMuConfig.SerializeValue(const AValue: TMuTomlValue;
  const ABuilder: TStringBuilder);
var
  LArray: TMuTomlArray;
  LI:     Integer;
...
procedure TMuConfig.SerializeToml(const AToml: TMuToml;
  const ABuilder: TStringBuilder; const AIndent: Integer;
  const ATablePath: string);
var
  LKeys:     TArray<string>;
...
function TMuConfig.SaveToFile(const AFilename: string): Boolean;
var
...
begin
  ...
  try
    ...
    try
      TMuUtils.CreateDirInPath(AFilename);
...
function TMuConfig.HasKey(const AKeyPath: string): Boolean;
var
  LValue: TMuTomlValue;
begin
...
function TMuConfig.GetString(const AKeyPath: string;
  const ADefault: string): string;
var
  LValue: TMuTomlValue;
begin
...
procedure TMuConfig.SetString(const AKeyPath: string;
  const AValue: string);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetInteger(const AKeyPath: string;
...
procedure TMuConfig.SetInteger(const AKeyPath: string;
  const AValue: Int64);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetFloat(const AKeyPath: string;
...
procedure TMuConfig.SetFloat(const AKeyPath: string;
  const AValue: Double);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetBoolean(const AKeyPath: string;
...
procedure TMuConfig.SetBoolean(const AKeyPath: string;
...
function TMuConfig.GetDateTime(const AKeyPath: string;
...
procedure TMuConfig.SetDateTime(const AKeyPath: string;
...
begin
  ...
  if NavigateToTable(AKeyPath, True, LTable, LFinalKey) then
    LTable.SetValue(LFinalKey, TMuTomlValue.CreateDateTime(AValue));
end;

// -- String Array -------------------------------------------------------------

function TMuConfig.GetStringArray(const AKeyPath: string): TArray<string>;
var
  LValue: TMuTomlValue;
  LArray: TMuTomlArray;
  LI:     Integer;
...
procedure TMuConfig.SetStringArray(const AKeyPath: string;
  const AValues: TArray<string>);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetIntegerArray(const AKeyPath: string): TArray<Int64>;
var
  LValue: TMuTomlValue;
  LArray: TMuTomlArray;
  LI:     Integer;
...
procedure TMuConfig.SetIntegerArray(const AKeyPath: string;
  const AValues: TArray<Int64>);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetFloatArray(const AKeyPath: string): TArray<Double>;
var
  LValue: TMuTomlValue;
  LArray: TMuTomlArray;
  LI:     Integer;
...
procedure TMuConfig.SetFloatArray(const AKeyPath: string;
  const AValues: TArray<Double>);
var
  LTable:    TMuToml;
  LFinalKey: string;
...
function TMuConfig.GetTableCount(const AKeyPath: string): Integer;
var
  LValue: TMuTomlValue;
begin
  Result := 0;
...
function TMuConfig.GetTableString(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: string): string;
var
  LValue:      TMuTomlValue;
...
function TMuConfig.GetTableInteger(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Int64): Int64;
var
  LValue:      TMuTomlValue;
...
function TMuConfig.GetTableFloat(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Double): Double;
var
  LValue:      TMuTomlValue;
...
function TMuConfig.GetTableBoolean(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Boolean): Boolean;
var
  LValue:      TMuTomlValue;
...
function TMuConfig.AddTableEntry(const AKeyPath: string): Integer;
var
  LTable:    TMuToml;
  LFinalKey: string;
  LArray:    TMuTomlArray;
...
procedure TMuConfig.SetTableString(const AKeyPath: string;
...
procedure TMuConfig.SetTableInteger(const AKeyPath: string;
...
procedure TMuConfig.SetTableFloat(const AKeyPath: string;
...
procedure TMuConfig.SetTableBoolean(const AKeyPath: string;
...
function TMuConfig.GetTableStringArray(const AKeyPath: string;
  const AIndex: Integer; const AField: string): TArray<string>;
var
  LValue:      TMuTomlValue;
  LArray:      TMuTomlArray;
...
procedure TMuConfig.SetTableStringArray(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const AValues: TArray<string>);
var
  LValue:      TMuTomlValue;
...
function TMuConfig.GetFileComment(): string;
begin
  if Assigned(FToml) then
...
procedure TMuConfig.SetFileComment(const AComment: string);
begin
  if not Assigned(FToml) then
    FToml := TMuToml.Create();
...
function TMuConfig.GetTableComment(const AKeyPath: string;
  const AIndex: Integer): string;
var
  LValue: TMuTomlValue;
  LArray: TMuTomlArray;
...
procedure TMuConfig.SetTableComment(const AKeyPath: string;
  const AIndex: Integer; const AComment: string);
var
  LValue: TMuTomlValue;
  LArray: TMuTomlArray;
