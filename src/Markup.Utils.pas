...
type
  ...
  { TMuCallback }
  TMuCallback<T> = record
    Callback: T;
    UserData: Pointer;
    function IsAssigned(): Boolean;
  end;

  { TMuCaptureConsoleCallback }
  TMuCaptureConsoleCallback = reference to procedure(const ALine: string; const AUserData: Pointer);

  { TMuStatusCallback }
  TMuStatusCallback = reference to procedure(const AText: string; const AUserData: Pointer);

  { TMuVersionInfo }
  TMuVersionInfo = record
    Major: Word;
    Minor: Word;
    Patch: Word;
    Build: Word;
  ...
  public
    ...
    class procedure CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TMuCaptureConsoleCallback); static;
    class procedure CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TMuCaptureConsoleCallback); static;
    ...
    class function  GetVersionInfo(out AVersionInfo: TMuVersionInfo; const AFilePath: string = ''): Boolean; static;
  ...
  { TMuErrorRelated }
  TMuErrorRelated = record
    Range: TMuSourceRange;
    Message: string;
  end;
  ...
  TMuError = record
    Range: TMuSourceRange;
    Severity: TMuErrorSeverity;
    Code: string;
    Message: string;
    Related: TArray<TMuErrorRelated>;
  ...
  { TMuErrorNotify — callback fired each time an error is added }
  TMuErrorNotify = reference to procedure(const AError: TMuError);

  { TMuErrors }
  TMuErrors = class
  private
    FItems: TList<TMuError>;
    FMaxErrors: Integer;
    FOnAdd: TMuErrorNotify;
  ...
  public
    ...
    procedure Add(
      const ARange: TMuSourceRange;
      const ASeverity: TMuErrorSeverity;
      const ACode: string;
    ...
    function GetItems(): TList<TMuError>;
    function GetMaxErrors(): Integer;
    procedure SetMaxErrors(const AMaxErrors: Integer);
    function ToString(): string; override;

    property OnAdd: TMuErrorNotify read FOnAdd write FOnAdd;
  end;
  ...
  protected
    FErrors: TMuErrors;
    FOwnsErrors: Boolean;
    FStatusCallback: TMuCallback<TMuStatusCallback>;
  public
    constructor Create(); virtual;
    destructor Destroy(); override;
    ...
    function  GetStatusCallback(): TMuStatusCallback;
    procedure SetStatusCallback(const ACallback: TMuStatusCallback; const AUserData: Pointer = nil); virtual;
    procedure SetErrors(const AErrors: TMuErrors); virtual;
    function  GetErrors(): TMuErrors;
    procedure PropagateErrors(const AChild: TMuBaseObject);
  end;

  { TMuCommandBuilder }
  TMuCommandBuilder = class(TMuBaseObject)
  private
    FParams: TStringList;
  public
...
function TMuCallback<T>.IsAssigned(): Boolean;
begin
...
class function TMuUtils.EnableVirtualTerminalProcessing(): Boolean;
var
  HOut: THandle;
  LMode: DWORD;
begin
...
class procedure TMuUtils.InitConsole();
begin
...
class procedure TMuUtils.FailIf(const Cond: Boolean; const Msg: string; const AArgs: array of const);
begin
  if Cond then
    raise Exception.CreateFmt(Msg, AArgs);
end;

class function TMuUtils.GetTickCount(): DWORD;
begin
  Result := WinApi.Windows.GetTickCount();
end;

class function TMuUtils.GetTickCount64(): UInt64;
begin
  Result := WinApi.Windows.GetTickCount64();
end;

class function TMuUtils.HasConsole(): Boolean;
begin
  Result := Boolean(GetConsoleWindow() <> 0);
end;

class procedure TMuUtils.ClearToEOL();
begin
  if not HasConsole() then Exit;
  Write(#27'[0K');
end;

class procedure TMuUtils.Print();
begin
  Print('');
end;

class procedure TMuUtils.PrintLn();
begin
  PrintLn('');
end;

class procedure TMuUtils.Print(const AText: string);
begin
  if not HasConsole() then Exit;
  Write(AText + COLOR_RESET);
end;

class procedure TMuUtils.Print(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  Write(Format(AText, AArgs) + COLOR_RESET);
end;

class procedure TMuUtils.PrintLn(const AText: string);
begin
  if not HasConsole() then Exit;
  WriteLn(AText + COLOR_RESET);
end;

class procedure TMuUtils.PrintLn(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  WriteLn(Format(AText, AArgs) + COLOR_RESET);
end;

class function TMuUtils.RGB(const AR, AG, AB: Byte): string;
begin
  Result := #27'[38;2;' + IntToStr(AR) + ';' + IntToStr(AG) + ';' + IntToStr(AB) + 'm';
end;

class function TMuUtils.Pause(const AMsg, AQuit: string): Boolean;
var
  LInput: string;
begin
  Result := False;
...
class function TMuUtils.AsUTF8(const AValue: string; ALength: PCardinal): Pointer;
begin
...
class function TMuUtils.ToAnsi(const AValue: string): AnsiString;
var
  LBytes: TBytes;
begin
...
class function TMuUtils.FromUtf8(const APtr: PAnsiChar): string;
begin
  if APtr = nil then
    Result := ''
  else
...
class procedure TMuUtils.ProcessMessages();
var
  LMsg: TMsg;
begin
...
class function TMuUtils.RunPE(const AExe, AParams, AWorkDir: string; const AWait: Boolean; const AShowCmd: Word): Cardinal;
var
  LAppPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class function TMuUtils.WindowsPathToWSL(const APath: string): string;
var
  LFullPath: string;
  LDrive: Char;
begin
...
class function TMuUtils.RunElf(const AElf, AWorkDir: string): Cardinal;
var
  LWslPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class procedure TMuUtils.CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TMuCaptureConsoleCallback);
const
  CReadBuffer = 1024 * 2;
var
...
class procedure TMuUtils.CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TMuCaptureConsoleCallback);
const
  CReadBuffer = 4096;
var
...
class function TMuUtils.CreateProcessWithPipes(const AExe, AParams, AWorkDir: string; out AStdinWrite: THandle; out AStdoutRead: THandle; out AProcessHandle: THandle; out AThreadHandle: THandle): Boolean;
var
...
class function TMuUtils.CreateDirInPath(const AFilename: string): Boolean;
var
  LPath: string;
begin
...
class procedure TMuUtils.CopyFilePreservingEncoding(const ASourceFile, ADestFile: string);
var
  LSourceBytes: TBytes;
begin
...
class function TMuUtils.DetectFileEncoding(const AFilePath: string): TEncoding;
var
  LBytes: TBytes;
  LEncoding: TEncoding;
begin
...
class function TMuUtils.EnsureBOM(const AText: string): string;
const
  UTF16_BOM = #$FEFF;
begin
  Result := AText;
...
class function TMuUtils.EscapeString(const AText: string): string;
var
  LI: Integer;
  LChar: Char;
  LNextChar: Char;
...
class function TMuUtils.StripAnsi(const AText: string): string;
var
...
class function TMuUtils.ExtractAnsiCodes(const AText: string): string;
var
  LResult: TStringBuilder;
  LIdx: Integer;
  LLen: Integer;
...
class function TMuUtils.GetVersionInfo(out AVersionInfo: TMuVersionInfo; const AFilePath: string): Boolean;
var
  LFileName: string;
  LInfoSize: DWORD;
  LHandle: DWORD;
...
class function TMuUtils.GetModuleVersionString(const AModule: HMODULE): string;
var
  LPath: array[0..MAX_PATH] of Char;
  LInfo: TMuVersionInfo;
begin
...
class function TMuUtils.IsValidWin64PE(const AFilePath: string): Boolean;
var
  LFile: TFileStream;
...
class procedure TMuUtils.UpdateIconResource(const AExeFilePath, AIconFilePath: string);
type
...
class procedure TMuUtils.UpdateVersionInfoResource(const PEFilePath: string; const AMajor, AMinor, APatch: Word; const AProductName, ADescription, AFilename, ACompanyName, ACopyright: string; const AURL: string);
type
...
class function TMuUtils.ResourceExist(const AResName: string): Boolean;
begin
  Result := Boolean((FindResource(HInstance, PChar(AResName), RT_RCDATA) <> 0));
end;

class function TMuUtils.AddResManifestFromResource(const AResName: string; const AModuleFile: string; ALanguage: Integer): Boolean;
var
  LHandle: THandle;
...
class procedure TMuUtils.UpdateRCDataResource(const AExeFilePath: string;
...
class function TMuUtils.GetFileSHA256(const APath: string): string;
begin
  Result := THashSHA2.GetHashStringFromFile(APath).ToLower();
end;

class function TMuUtils.GetRelativePath(const ABasePath, AFullPath: string): string;
var
  LBasePath: string;
  LFullPath: string;
  LBaseLen: Integer;
...
class function TMuUtils.NormalizePath(const APath: string): string;
begin
  Result := APath.Replace(PathDelim, '/');
end;

class function TMuUtils.DisplayPath(const APath: string): string;
begin
  Result := TPath.GetFullPath(APath).Replace('\', '/');
end;

class function TMuUtils.GetEnv(const AName: string): string;
begin
  Result := GetEnvironmentVariable(AName);
end;

class procedure TMuUtils.SetEnv(const AName: string; const AValue: string);
begin
  SetEnvironmentVariable(PChar(AName), PChar(AValue));
end;

class function TMuUtils.HasEnv(const AName: string): Boolean;
begin
  Result := not GetEnv(AName).IsEmpty();
end;

class function TMuUtils.RunFromIDE(): Boolean;
begin
  Result := HasEnv('BDS');
end;

class function TMuUtils.CountLines(
...
{ TMuSourceRange }
procedure TMuSourceRange.Clear();
begin
  Filename := '';
  StartLine := 0;
  StartColumn := 0;
...
function TMuSourceRange.IsEmpty(): Boolean;
begin
  Result := (StartLine = 0) and (StartColumn = 0);
end;

function TMuSourceRange.ToPointString(): string;
begin
  if IsEmpty() then
    Result := ''
  else
...
function TMuSourceRange.ToRangeString(): string;
begin
  if IsEmpty() then
    Result := ''
...
function TMuError.GetSeverityString(): string;
begin
  case Severity of
...
function TMuError.ToIDEString(): string;
begin
...
function TMuError.ToFullString(): string;
var
...
constructor TMuErrors.Create();
begin
  inherited;

  FItems := TList<TMuError>.Create();
  FMaxErrors := DEFAULT_MAX_ERRORS;
end;

destructor TMuErrors.Destroy();
begin
  FItems.Free();
...
function TMuErrors.CountErrors(): Integer;
var
  LError: TMuError;
begin
  Result := 0;
...
procedure TMuErrors.AddRelated(
  const ARange: TMuSourceRange;
  const AMessage: string
);
var
  LError: TMuError;
  LRelated: TMuErrorRelated;
  LLen: Integer;
begin
...
function TMuErrors.HasHints(): Boolean;
var
  LError: TMuError;
begin
  Result := False;
...
function TMuErrors.HasWarnings(): Boolean;
var
  LError: TMuError;
begin
  Result := False;
...
function TMuErrors.HasErrors(): Boolean;
var
  LError: TMuError;
begin
  Result := False;
...
function TMuErrors.HasFatal(): Boolean;
var
  LError: TMuError;
begin
  Result := False;
...
function TMuErrors.Count(): Integer;
begin
  Result := FItems.Count;
end;

function TMuErrors.ErrorCount(): Integer;
begin
  Result := CountErrors();
end;

function TMuErrors.WarningCount(): Integer;
var
  LError: TMuError;
begin
  Result := 0;
...
function TMuErrors.ReachedMaxErrors(): Boolean;
begin
  Result := CountErrors() >= FMaxErrors;
end;

procedure TMuErrors.Clear();
begin
  FItems.Clear();
end;

procedure TMuErrors.TruncateTo(const ACount: Integer);
begin
  while FItems.Count > ACount do
    FItems.Delete(FItems.Count - 1);
end;

function TMuErrors.GetItems(): TList<TMuError>;
begin
  Result := FItems;
end;

function TMuErrors.GetMaxErrors(): Integer;
begin
  Result := FMaxErrors;
end;

procedure TMuErrors.SetMaxErrors(const AMaxErrors: Integer);
begin
  FMaxErrors := AMaxErrors;
end;

function TMuErrors.ToString(): string;
var
  LBuilder: TStringBuilder;
  LI: Integer;
begin
...
constructor TMuBaseObject.Create();
begin
  inherited;
  FErrors := TMuErrors.Create();
  FOwnsErrors := True;
end;

destructor TMuBaseObject.Destroy();
begin
...
function TMuBaseObject.Dump(const AId: Integer): string;
begin
  Result := '';
end;

procedure TMuBaseObject.InitConfig();
begin
end;

procedure TMuBaseObject.LoadConfig(const AFilename: string);
begin
end;

procedure TMuBaseObject.SaveConfig(const AFilename: string);
begin
end;


procedure TMuBaseObject.Status(const AText: string);
begin
  if FStatusCallback.IsAssigned() then
    FStatusCallback.Callback(AText, FStatusCallback.UserData);
end;

procedure TMuBaseObject.Status(const AText: string; const AArgs: array of const);
begin
  Status(Format(AText, AArgs));
end;

function TMuBaseObject.GetStatusCallback(): TMuStatusCallback;
begin
  Result := FStatusCallback.Callback;
end;

procedure TMuBaseObject.SetStatusCallback(const ACallback: TMuStatusCallback; const AUserData: Pointer);
begin
  FStatusCallback.Callback := ACallback;
  FStatusCallback.UserData := AUserData;
end;

procedure TMuBaseObject.SetErrors(const AErrors: TMuErrors);
begin
...
function TMuBaseObject.GetErrors(): TMuErrors;
begin
  Result := FErrors;
end;

procedure TMuBaseObject.PropagateErrors(const AChild: TMuBaseObject);
begin
...
constructor TMuCommandBuilder.Create();
begin
  inherited;
...
destructor TMuCommandBuilder.Destroy();
begin
  FreeAndNil(FParams);
...
procedure TMuCommandBuilder.Clear();
begin
  FParams.Clear();
end;

procedure TMuCommandBuilder.AddParam(const AParam: string);
begin
  if AParam <> '' then
    FParams.Add(AParam);
end;

procedure TMuCommandBuilder.AddParam(const AFlag, AValue: string);
begin
  if AFlag <> '' then
  begin
    if AValue <> '' then
...
procedure TMuCommandBuilder.AddQuotedParam(const AFlag, AValue: string);
begin
  if AValue = '' then
    Exit;
...
procedure TMuCommandBuilder.AddQuotedParam(const AValue: string);
begin
  AddQuotedParam('', AValue);
end;

procedure TMuCommandBuilder.AddFlag(const AFlag: string);
begin
  if AFlag <> '' then
    FParams.Add(AFlag);
end;

function TMuCommandBuilder.Dump(const AId: Integer): string;
var
  LI: Integer;
begin
...
function TMuCommandBuilder.GetParamCount(): Integer;
begin
...
begin
  ...
  TMuUtils.InitConsole();
  {$IFDEF VPR_LEAK_TRACK}
  TMuBaseObject.InitLeakTracking();
  {$ENDIF}
end;
...
begin
  {$IFDEF VPR_LEAK_TRACK}
  TMuBaseObject.FinalizeLeakTracking();
  {$ENDIF}
end;
